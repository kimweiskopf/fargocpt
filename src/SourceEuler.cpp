...
static void thermal_relaxation(t_data &data, const double current_time) {
	...
	for (unsigned int nr = 1; nr < Nr; ++nr) {
		...
		if (parameters::approx_beta_surf) {
		  ...
		  const double G = constants::G.get_code_value();
		  const double M_star = planet.get_mass();
		  
		  const double distance_measured = std::sqrt(std::pow(x - xc, 2) + std::pow(y - yc, 2));
		  const double distance = std::max(distance_measured, min_dist); // richtige units????	      
		  
		  // define T_0
		  const double T_0 = std::pow( ( 1-0.5) * std::pow(T_star,4)* std::pow(R_star,2)* 2/7 * std::pow(Rgas/(mu*G*M_star),1/2)), 2/7) * std::pow(distance,-3/7);
		  
		  //define tau_eff bei T_0
		  const double tau_0 = 2.e-4* sigma/std::pow(8*M_PI,1/2); //richtige units????
		  const double tau_eff =3*tau_0*std::pow(T_0,2)/8 + 1/2 +1/(4*tau_0*std::pow(T_0,2)- tau_min);
		  
		  
		  //define the derivation of tau_eff
      
		  const double d_tau_eff = 2*std::pow(T_0,2)* (tau_0 *3/8 - 4*tau_0/std::pow(4*tau_0*std::pow(T_0,2) - tau_min,2)) *1/tau_eff;
		  
		  
		  // define Q_surf factor 2 sigma_sb T^4 / tau_eff
	          const double qsurf = factor * 2 * sigma_sb * (std::pow(T_0,4) - Tmin4) / tau_eff;

              
                  // define e_thermal
		  const double e_thermal = c_v * T_0 * sigma;
		  
		  const double t_cool = e_thermal/qsurf;
		  
		  beta_inv = std::pow(omega_k * t_cool * (4 - d_tau_eff),-1)
